 ...
 *      |-----------|
 */
int loc_init(unsigned int ab, unsigned int ac, unsigned int bc) //cm
{
  TRIANGLE_TYPE type = is_triangle(ab, ac, bc);
  if (type == NONE_TRIANGLE)   return -1;
  uwb_PS.c.loc_cm.y = uwb_PS.b.loc_cm.y = uwb_PS.b1.loc_cm.y = side_get_high_cm(ab/100.0, ac/100.0, bc/100.0);
 ...
 */
TRIANGLE_TYPE is_triangle(unsigned int const L1_cm, unsigned int const L2_cm, unsigned int const L3_cm)
{
 ...
 */
double Heron_formula(double const L1_m, double const L2_m, double const L3_m)
{
  double p_m = (L1_m + L2_m + L3_m) / 2;
  return uwbSqrt(p_m * (p_m - L1_m) * (p_m - L2_m) * (p_m - L3_m));
}
//面积求高
inline double area_get_high(double const S_m2, double const w_m)
{
  return 2 * S_m2 / w_m;
}

//三边求在L3上的高 返回米
inline double side_get_high_m(double const L1_m, double const L2_m, double const L3_m)
{
  return area_get_high(Heron_formula(L1_m, L2_m, L3_m), L3_m);
}

//三边求在L3上的高 返回厘米
inline int side_get_high_cm(double const L1_m, double const L2_m, double const L3_m)
{
  return (int)(area_get_high(Heron_formula(L1_m, L2_m, L3_m), L3_m)*100);
}

//勾股定理返回直边之一 返回米
inline double  triangle_get_side_m(double hypotenuse_m, double side_m)
{
  return uwbSqrt(hypotenuse_m * hypotenuse_m - side_m * side_m);
}

//勾股定理返回直边之一 返回厘米
inline int triangle_get_side_cm(double hypotenuse_m, double side_m)
{
  return (int)(uwbSqrt(hypotenuse_m * hypotenuse_m - side_m * side_m)*100);
}

//勾股定理返回斜边 返回米
inline double  triangle_get_hypotenuse_m(double side1_m, double side2_m)
{
  return uwbSqrt(side1_m * side1_m + side2_m * side2_m);
}

//勾股定理返回斜边 返回厘米
inline int triangle_get_hypotenuse_cm(double side1_m, double side2_m)
{
